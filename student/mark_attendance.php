...
if ($is_lecturer && (!$week_number || !$lecture_name)) json_error('Invalid lecturer session.');

if ($is_lecturer) {
    $sess = $conn->prepare("SELECT id, lat, lng, radius_m, week_number, lecture_name FROM qr_sessions WHERE lecturer_id = ? AND code = ? AND ended_at IS NULL LIMIT 1");
...
if ($is_lecturer) {
    ...
    $dup->execute();
    if ($dup->get_result()->num_rows > 0) json_error('You have already marked attendance for this week.');
    ...
    $null_cr = null;
    $ins = $conn->prepare("INSERT INTO attendance (student_id, classrep_id, lecturer_id, student_name, index_number, attendance_date, time_marked, lecture_name, week_number, device_id, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
    $ins->bind_param('iiisssssiss', $student['id'], $null_cr, $lecturer_id, $student['name'], $index_number, $today, $time_marked, $lecture_name, $week_number, $device_id, $location_status);
} else {
...
try {
    ...
    $label = $is_lecturer ? "Week $week_number: $lecture_name" : $lecture_name;
