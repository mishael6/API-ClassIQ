...
if (!$student) json_error('Student not found. Make sure the student is registered under your class.');
...
if ($existing) {
    ...
    json_error('Student is already marked for this lecture.');
}

$time_marked = date('H:i:s');
$device_id   = 'Manual Add by Class Rep';

// Fixed: 8 bind vars = 'iissssss'
...
$ins->bind_param('iissssss', $student['id'], $classrep_id, $student['name'], $index_number, $attendance_date, $time_marked, $lecture_name, $device_id);

if (!$ins->execute()) json_error('Failed to add attendance: ' . $conn->error);
json_ok(['message' => 'Student added to attendance successfully.', 'success' => true]);
